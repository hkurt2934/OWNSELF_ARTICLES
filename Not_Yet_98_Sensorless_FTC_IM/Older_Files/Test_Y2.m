...
enable_trc = true;
mu_trc = 5e-4;          % adaptation gain (try 1e-4..2e-3)
trc_lim = 0.25;         % max |harmonic coefficient| (per-unit of fundamental)
tau_Te_lp = 0.05;       % [s] low-pass time constant for torque mean
    ...
    for k = 1:N
        ...
        if ~fault
            ...
            if ftc_active
                ...
                h2 = c_s2*sin(2*theta) + c_c2*cos(2*theta);
                % keep harmonic bounded
                h2 = min(max(h2, -trc_lim), trc_lim);
                v_bc = sqrt(3)*Vpk*(sin(theta) + h2);
            else
        ...
        if fault && ftc_active && enable_trc
            ...
            e_tr = Te_lp - Te;   % negative of instantaneous ripple
            % Adapt 2nd-harmonic coefficients (LMS)
            x_s2 = sin(2*theta);
            x_c2 = cos(2*theta);
            c_s2 = c_s2 + mu_trc * e_tr * x_s2;
            c_c2 = c_c2 + mu_trc * e_tr * x_c2;
            % Clamp coefficients
