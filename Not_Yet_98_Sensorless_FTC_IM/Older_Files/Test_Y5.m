%% Test_X1_sensorless_FTC_MRAS_v3.m
...
tfault = 1.0;
TL = 50;                      % N.m
w_eps  = 5*2*pi/60;     % smooth sign() for load torque (5 rpm threshold)
...
% Torque-ripple compensator (FTC enhancement)
% Adds a small 2*w_s harmonic to v_bc in FTC mode and adapts its amplitude
% to reduce the 2nd-harmonic torque pulsation (simple LMS).
enable_trc = true;
mu_trc = 2.0e-3;          % adaptation gain (strengthened; flip sign in update below)
trc_lim = 0.35;          % max |harmonic coefficient| (p.u. of fundamental)
tau_Te_lp = 0.03;        % [s] mean torque LPF time constant

t_cl   = 0.70;           % keep longer open-loop so flux builds + MRAS settles            % [s] open-loop start-up ramp time

tau_corr = 0.02;        % [s] correlation LPF for torque ripple canceller

%% ================== Sensorless MRAS Parameters ==================
% Voltage-model flux integrator leakage to prevent drift
kf_flux = 0.5;           % smaller leakage to avoid collapsing flux estimate                % rad/s (small)

% MRAS adaptation gains (electrical rotor speed)
% Error is normalized => gains can be moderate.
Kp_mras = 120.0;        % MRAS adaptation gain (P)
Ki_mras = 3000.0;       % MRAS adaptation gain (I)
wr_hat_min = -ws_max;     % allow reverse estimate (prevents clamping at 0)
wr_hat_max = ws_max;
...
function out = sim_case(opt)
    ...
    enable_trc = evalin('base','enable_trc');
    mu_trc     = evalin('base','mu_trc');
    trc_lim    = evalin('base','trc_lim');
    tau_Te_lp  = evalin('base','tau_Te_lp');
    t_cl = evalin('base','t_cl');
    tau_corr = evalin('base','tau_corr');
    ...
    int_mras = 0;
    wr_hat = 0.9*ws_ref;  % electrical rotor speed estimate (init guess)
    ...
    fcnt = 0;
    ftc_active = false;
    % Torque ripple compensator states (FTC mode)
    c_s2 = 0; c_c2 = 0;      % sin(2θ), cos(2θ) coefficients (p.u.)
    Te_lp = 0;              % low-pass torque estimate [N.m]
    d_s2 = 0; d_c2 = 0;      % demodulated ripple components
    ...
    for k = 1:N
        ...
        % Select control gains (FTC mode vs normal)
        if ftc_active
        ...
        % Start-up: open-loop ramp to avoid estimator/PI windup at t=0.
        if t < t_cl
            ws_cmd = min(max(ws_ref*(t/t_cl), ws_min), ws_max);
            int_w = 0;
        else
            % Closed-loop speed PI (uses estimated speed)
            e_w = w_ref - wm_hat;
            int_w = int_w + e_w*Ts;
        ...
        if ~fault
            ...
            if ftc_active
                % FTC mode: direct two-leg modulation with adaptive 2*theta ripple cancellation
                v_bc = sqrt(3)*Vpk*( sin(theta) + c_s2*sin(2*theta) + c_c2*cos(2*theta) );
            else
        ...
        % ===== MRAS update (rotor-flux MRAS) =====
        % Voltage-model stator flux (with small leakage term to avoid drift)
        psi_s_v = psi_s_v + Ts*(v_s - Rs*i_s - kf_flux*psi_s_v);

        % Reference rotor flux from voltage-model stator flux
        % psi_r_ref = (Lr/Lm) * (psi_s - sigma*Ls*i_s)
        psi_r_ref = (Lr/Lm) * (psi_s_v - sigma*Ls*i_s);

        % Current-model rotor flux (depends on wr_hat)
        dpsi_r_i = (Lm/Tr)*i_s - (1/Tr)*psi_r_i + 1j*wr_hat*psi_r_i;
        psi_r_i  = psi_r_i + Ts*dpsi_r_i;

        % MRAS error (rotor-flux MRAS):
        % normalized cross-product; sign chosen to prevent estimator collapsing to 0
        denom = (abs(psi_r_ref)^2) + 1e-6;  % normalize by |psi_r_ref|^2
        e_mras_raw = imag( psi_r_i * conj(psi_r_ref) )/denom;

        % Enable adaptation only after flux is established (startup) to avoid false locking
        if (t >= t_cl) && (abs(psi_r_ref) > 0.12) && (abs(psi_r_i) > 0.12)
            e_mras = max(min(e_mras_raw, 1), -1);

            % PI adaptation (electrical rotor speed)
            int_mras = int_mras + e_mras*Ts;
            int_mras = max(min(int_mras, 1.0), -1.0);  % anti-windup
            wr_hat = wr_hat + Ts*(Kp_mras*e_mras + Ki_mras*int_mras);
            wr_hat = min(max(wr_hat, wr_hat_min), wr_hat_max);
        else
            % during startup: keep a reasonable guess tied to synchronous speed
            wr_hat = 0.95*ws_cmd;
            int_mras = 0;
        end
        ...
        % ===== Torque ripple compensator update (FTC only) =====
        if fault && ftc_active && enable_trc
            % Low-pass mean torque
            a_lp = Ts/(tau_Te_lp + Ts);
            Te_lp = Te_lp + a_lp*(Te - Te_lp);

            % Ripple component
            e_r = Te - Te_lp;

            % Synchronous demodulation at 2*theta + correlation LPF
            a_corr = Ts/(tau_corr + Ts);
            d_s2 = d_s2 + a_corr*(e_r*sin(2*theta) - d_s2);
            d_c2 = d_c2 + a_corr*(e_r*cos(2*theta) - d_c2);

            % Gradient step to cancel 2nd-harmonic ripple
            c_s2 = c_s2 - mu_trc * d_s2;
            c_c2 = c_c2 - mu_trc * d_c2;

            % Clamp coefficients
            c_s2 = min(max(c_s2, -trc_lim), trc_lim);
            c_c2 = min(max(c_c2, -trc_lim), trc_lim);
        end
        TL_eff = TL * tanh(wm / w_eps);
        wm = wm + (Te - TL_eff - B*wm)/J * Ts;
