...
enable_trc = true;
mu_trc = 1.2e-3;          % adaptation gain (try 5e-4..3e-3)
...
function out = sim_case(opt)
    ...
    for k = 1:N
        ...
        % MRAS error: cross-product between reference and estimated rotor flux
        denom = max(abs(psi_r_ref)^2, 1e-6);
        e_mras = imag( psi_r_ref * conj(psi_r_i) )/denom;
